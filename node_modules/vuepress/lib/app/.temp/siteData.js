export const siteData = {
  "title": "",
  "description": "",
  "base": "/myblog/",
  "pages": [
    {
      "key": "v-26644fb9f73f7",
      "path": "/",
      "lastUpdated": 1550055908000,
      "title": "Home",
      "frontmatter": {
        "home": true,
        "heroImage": "/baobao.jpeg",
        "heroText": "李加廷の博客",
        "tagline": "努力攒够了，梦想才会开花！🌼",
        "actionText": "梦想启程 →",
        "actionLink": "/springcloud/",
        "features": [
          {
            "title": "微服务架构学习",
            "details": "Spring Cloud是一系列框架的有序集合。它利用Spring Boot的开发便利性巧妙地简化了分布式系统基础设施的开发，如服务发现注册、配置中心、消息总线、负载均衡、断路器、数据监控等，都可以用Spring Boot的开发风格做到一键启动和部署。"
          },
          {
            "title": "Vue前端学习",
            "details": "Vue是一套用于构建用户界面的渐进式框架。与其它大型框架不同的是，Vue 被设计为可以自底向上逐层应用。Vue 的核心库只关注视图层，不仅易于上手，还便于与第三方库或既有项目整合。另一方面，当与现代化的工具链以及各种支持类库结合使用时，Vue 也完全能够为复杂的单页应用提供驱动。"
          },
          {
            "title": "Flutter移动端学习",
            "details": "Flutter是谷歌的移动UI框架，可以快速在iOS和Android上构建高质量的原生用户界面。 Flutter可以与现有的代码一起工作。在全世界，Flutter正在被越来越多的开发者和组织使用，并且Flutter是完全免费、开源的。"
          }
        ],
        "footer": "MIT Licensed | Copyright © 2019-present Jerry You"
      }
    },
    {
      "key": "v-078da1b8dbe6a",
      "path": "/about/",
      "lastUpdated": 1549859269000,
      "title": "个人信息",
      "headers": [
        {
          "level": 2,
          "title": "个人信息",
          "slug": "个人信息"
        },
        {
          "level": 2,
          "title": "链接",
          "slug": "链接"
        },
        {
          "level": 2,
          "title": "联系方式",
          "slug": "联系方式"
        },
        {
          "level": 2,
          "title": "我的简历",
          "slug": "我的简历"
        },
        {
          "level": 2,
          "title": "参考技能关键字",
          "slug": "参考技能关键字"
        }
      ]
    },
    {
      "key": "v-f8adc9b7e7193",
      "path": "/css/",
      "lastUpdated": 1549859269000,
      "title": "开启CSS学习之路",
      "frontmatter": {
        "title": "开启CSS学习之路"
      }
    },
    {
      "key": "v-1615bdc95351a",
      "path": "/css/css1.html",
      "lastUpdated": 1549964418000,
      "title": "css学习第一篇",
      "frontmatter": {
        "title": "css学习第一篇",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "css学习第一篇"
          },
          {
            "name": "keywords",
            "content": "css学习第一篇"
          }
        ]
      }
    },
    {
      "key": "v-c344b7791dbe6",
      "path": "/css/css2.html",
      "lastUpdated": 1549859269000,
      "title": "css学习第二篇",
      "frontmatter": {
        "title": "css学习第二篇"
      }
    },
    {
      "key": "v-0694754b318f",
      "path": "/en/%E8%8B%B1%E6%96%87%E7%9B%AE%E5%BD%95.html",
      "lastUpdated": 1549964418000
    },
    {
      "key": "v-d8dad498c3dcd",
      "path": "/java/",
      "lastUpdated": 1549859269000,
      "title": "开始Java学习之路",
      "frontmatter": {
        "title": "开始Java学习之路"
      }
    },
    {
      "key": "v-db97a9cbacdf9",
      "path": "/java/java1.html",
      "lastUpdated": 1549964418000,
      "title": "java学习第一篇",
      "headers": [
        {
          "level": 2,
          "title": "斤斤计较军",
          "slug": "斤斤计较军"
        },
        {
          "level": 3,
          "title": "就看看扩扩",
          "slug": "就看看扩扩"
        }
      ],
      "frontmatter": {
        "title": "java学习第一篇",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "快应用开发资源、教程汇聚，云集最新的快应用教程文章、开发资源、项目案例及新闻动态；为快应用开发者提供便利和参考。"
          },
          {
            "name": "keywords",
            "content": "快应用,资源,教程,开发快应用,quickapp,快应用开发者,新闻动态"
          }
        ]
      }
    },
    {
      "key": "v-e19483c1751c1",
      "path": "/java/java2.html",
      "lastUpdated": 1549859269000,
      "title": "java学习第二篇",
      "headers": [
        {
          "level": 2,
          "title": "斤斤计较军",
          "slug": "斤斤计较军"
        },
        {
          "level": 3,
          "title": "就看看扩扩",
          "slug": "就看看扩扩"
        }
      ],
      "frontmatter": {
        "title": "java学习第二篇"
      }
    },
    {
      "key": "v-813e851aac844",
      "path": "/js/",
      "lastUpdated": 1549859269000,
      "title": "开启JS学习之路",
      "frontmatter": {
        "title": "开启JS学习之路"
      }
    },
    {
      "key": "v-6279787fc58f1",
      "path": "/js/js1.html",
      "lastUpdated": 1549859269000,
      "title": "js学习第一课",
      "frontmatter": {
        "title": "js学习第一课"
      }
    },
    {
      "key": "v-265de6688b8c9",
      "path": "/js/js2.html",
      "lastUpdated": 1549859269000,
      "title": "js学习第二课",
      "frontmatter": {
        "title": "js学习第二课"
      }
    },
    {
      "key": "v-a7cc3abc30aa1",
      "path": "/otherProse/",
      "lastUpdated": null,
      "title": "我喜欢的散文文章",
      "frontmatter": {
        "title": "我喜欢的散文文章"
      }
    },
    {
      "key": "v-1ef201680869f",
      "path": "/otherProse/gogsInstall.html",
      "lastUpdated": null,
      "title": "gogs安装",
      "headers": [
        {
          "level": 2,
          "title": "环境要求",
          "slug": "环境要求"
        },
        {
          "level": 2,
          "title": "数据库安装",
          "slug": "数据库安装"
        },
        {
          "level": 3,
          "title": "这里以安装MySQL5.7为例",
          "slug": "这里以安装mysql5-7为例"
        },
        {
          "level": 3,
          "title": "1.下载tar包，这里使用wget从官网下载",
          "slug": "_1-下载tar包，这里使用wget从官网下载"
        },
        {
          "level": 3,
          "title": "2.将mysql安装到/usr/local/mysql下",
          "slug": "_2-将mysql安装到-usr-local-mysql下"
        },
        {
          "level": 3,
          "title": "3.新建data目录",
          "slug": "_3-新建data目录"
        },
        {
          "level": 3,
          "title": "4.新建mysql用户、mysql用户组",
          "slug": "_4-新建mysql用户、mysql用户组"
        },
        {
          "level": 3,
          "title": "5.将/usr/local/mysql的所有者及所属组改为mysql",
          "slug": "_5-将-usr-local-mysql的所有者及所属组改为mysql"
        },
        {
          "level": 3,
          "title": "6.配置",
          "slug": "_6-配置"
        },
        {
          "level": 3,
          "title": "7.开启服务",
          "slug": "_7-开启服务"
        },
        {
          "level": 3,
          "title": "8.设置密码",
          "slug": "_8-设置密码"
        },
        {
          "level": 3,
          "title": "9.将/etc/my.cnf中的skip-grant-tables删除",
          "slug": "_9-将-etc-my-cnf中的skip-grant-tables删除"
        },
        {
          "level": 3,
          "title": "10.登录再次设置密码（不知道为啥如果不再次设置密码就操作不了数据库了）",
          "slug": "_10-登录再次设置密码（不知道为啥如果不再次设置密码就操作不了数据库了）"
        },
        {
          "level": 3,
          "title": "11.允许远程连接",
          "slug": "_11-允许远程连接"
        },
        {
          "level": 3,
          "title": "12.添加快捷方式",
          "slug": "_12-添加快捷方式"
        },
        {
          "level": 2,
          "title": "安装其它要求",
          "slug": "安装其它要求"
        },
        {
          "level": 3,
          "title": "Mac OS X",
          "slug": "mac-os-x"
        },
        {
          "level": 3,
          "title": "Debian/Ubuntu",
          "slug": "debian-ubuntu"
        },
        {
          "level": 3,
          "title": "Windows",
          "slug": "windows"
        },
        {
          "level": 2,
          "title": "安装 Gogs",
          "slug": "安装-gogs"
        }
      ],
      "frontmatter": {
        "text": "gogs安装教程",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "gogs安装教程"
          },
          {
            "name": "keywords",
            "content": "gogs安装教程"
          }
        ]
      }
    },
    {
      "key": "v-1b395542b6c45",
      "path": "/otherProse/markdown%E6%89%A9%E5%B1%95.html",
      "lastUpdated": null,
      "title": "开始练习",
      "headers": [
        {
          "level": 2,
          "title": "开始练习",
          "slug": "开始练习"
        },
        {
          "level": 2,
          "title": "h2 Heading",
          "slug": "h2-heading"
        },
        {
          "level": 3,
          "title": "h3 Heading",
          "slug": "h3-heading"
        },
        {
          "level": 2,
          "title": "Horizontal Rules",
          "slug": "horizontal-rules"
        },
        {
          "level": 2,
          "title": "Typographic replacements",
          "slug": "typographic-replacements"
        },
        {
          "level": 2,
          "title": "Emphasis",
          "slug": "emphasis"
        },
        {
          "level": 2,
          "title": "Blockquotes",
          "slug": "blockquotes"
        },
        {
          "level": 2,
          "title": "Lists",
          "slug": "lists"
        },
        {
          "level": 2,
          "title": "Code",
          "slug": "code"
        },
        {
          "level": 2,
          "title": "Tables",
          "slug": "tables"
        },
        {
          "level": 2,
          "title": "Links",
          "slug": "links"
        },
        {
          "level": 2,
          "title": "Images",
          "slug": "images"
        },
        {
          "level": 2,
          "title": "Plugins",
          "slug": "plugins"
        },
        {
          "level": 3,
          "title": "Emojies",
          "slug": "emojies"
        },
        {
          "level": 3,
          "title": "Subscript](https://github.com/markdown-it/markdown-it-sub) / [Superscript",
          "slug": "subscript-superscript"
        },
        {
          "level": 3,
          "title": "\\",
          "slug": "ins"
        },
        {
          "level": 3,
          "title": "\\",
          "slug": "mark"
        },
        {
          "level": 3,
          "title": "Footnotes",
          "slug": "footnotes"
        },
        {
          "level": 3,
          "title": "Definition lists",
          "slug": "definition-lists"
        },
        {
          "level": 3,
          "title": "Abbreviations",
          "slug": "abbreviations"
        },
        {
          "level": 3,
          "title": "Custom containers",
          "slug": "custom-containers"
        }
      ],
      "frontmatter": {
        "text": "markdown扩展",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "markdown扩展"
          },
          {
            "name": "keywords",
            "content": "markdown扩展"
          }
        ]
      }
    },
    {
      "key": "v-3a51076209ab",
      "path": "/plan/2019plan.html",
      "lastUpdated": 1549964418000,
      "title": "2019，我的计划",
      "headers": [
        {
          "level": 2,
          "title": "学习计划",
          "slug": "学习计划"
        },
        {
          "level": 2,
          "title": "最终计划",
          "slug": "最终计划"
        }
      ],
      "frontmatter": {
        "title": "2019，我的计划",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "我的计划"
          },
          {
            "name": "keywords",
            "content": "2019我的计划"
          }
        ]
      }
    },
    {
      "key": "v-85829632c0f15",
      "path": "/plan/",
      "lastUpdated": 1549859269000,
      "title": "我的职业生涯计划导航",
      "frontmatter": {
        "title": "我的职业生涯计划导航"
      }
    },
    {
      "key": "v-2fac462df4331",
      "path": "/resources/books.html",
      "lastUpdated": 1549964418000,
      "title": "免费图书",
      "headers": [
        {
          "level": 2,
          "title": "翻墙",
          "slug": "翻墙"
        },
        {
          "level": 2,
          "title": "数据库",
          "slug": "数据库"
        },
        {
          "level": 2,
          "title": "Security 安全",
          "slug": "security-安全"
        },
        {
          "level": 2,
          "title": "Java",
          "slug": "java"
        },
        {
          "level": 2,
          "title": "Golang",
          "slug": "golang"
        },
        {
          "level": 2,
          "title": "Web",
          "slug": "web"
        },
        {
          "level": 2,
          "title": "Javascript",
          "slug": "javascript"
        },
        {
          "level": 2,
          "title": "NodeJS",
          "slug": "nodejs"
        },
        {
          "level": 2,
          "title": "CSS",
          "slug": "css"
        },
        {
          "level": 2,
          "title": "软件工程",
          "slug": "软件工程"
        },
        {
          "level": 2,
          "title": "项目管理",
          "slug": "项目管理"
        },
        {
          "level": 2,
          "title": "版本管理",
          "slug": "版本管理"
        },
        {
          "level": 2,
          "title": "工作流",
          "slug": "工作流"
        },
        {
          "level": 2,
          "title": "Linux",
          "slug": "linux"
        },
        {
          "level": 2,
          "title": "Mobile Development",
          "slug": "mobile-development"
        },
        {
          "level": 2,
          "title": "Swift",
          "slug": "swift"
        },
        {
          "level": 2,
          "title": "架构",
          "slug": "架构"
        },
        {
          "level": 2,
          "title": "云计算",
          "slug": "云计算"
        },
        {
          "level": 2,
          "title": "大数据",
          "slug": "大数据"
        },
        {
          "level": 2,
          "title": "操作系统",
          "slug": "操作系统"
        }
      ],
      "frontmatter": {
        "title": "免费图书",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "免费图书"
          },
          {
            "name": "keywords",
            "content": "免费图书"
          }
        ]
      }
    },
    {
      "key": "v-1aca7ee99f80e",
      "path": "/resources/resources.html",
      "lastUpdated": 1549964418000,
      "title": "资料链接",
      "headers": [
        {
          "level": 2,
          "title": "前端学习资料",
          "slug": "前端学习资料"
        },
        {
          "level": 2,
          "title": "node第三方库",
          "slug": "node第三方库"
        }
      ],
      "frontmatter": {
        "title": "资料链接",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "前端学习资源"
          },
          {
            "name": "keywords",
            "content": "前端学习资源"
          }
        ]
      }
    },
    {
      "key": "v-53d001af8b3d6",
      "path": "/springboot/",
      "lastUpdated": 1549859269000,
      "title": "springboot引你入门",
      "frontmatter": {
        "title": "springboot引你入门"
      }
    },
    {
      "key": "v-3bfb8e08b6215",
      "path": "/springboot/springboot1.html",
      "lastUpdated": 1549859269000,
      "title": "springboot学习第一课",
      "frontmatter": {
        "title": "springboot学习第一课"
      }
    },
    {
      "key": "v-ad61d7724bd7d",
      "path": "/springboot/springboot2.html",
      "lastUpdated": 1549859269000,
      "title": "springboot学习第二课",
      "frontmatter": {
        "title": "springboot学习第二课"
      }
    },
    {
      "key": "v-7f70bc2c4a37d",
      "path": "/springcloud/AwesomeSpringCloud.html",
      "lastUpdated": 1549964418000,
      "title": "收集各种Spring Cloud 学习资源",
      "headers": [
        {
          "level": 2,
          "title": "目录",
          "slug": "目录"
        },
        {
          "level": 2,
          "title": "博客",
          "slug": "博客"
        },
        {
          "level": 2,
          "title": "开源",
          "slug": "开源"
        },
        {
          "level": 2,
          "title": "网站",
          "slug": "网站"
        },
        {
          "level": 2,
          "title": "其它",
          "slug": "其它"
        },
        {
          "level": 3,
          "title": "谢谢",
          "slug": "谢谢"
        }
      ],
      "frontmatter": {
        "text": "SpringCloud学习资源",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "SpringCloud学习资源"
          },
          {
            "name": "keywords",
            "content": "SpringCloud学习资源"
          }
        ]
      }
    },
    {
      "key": "v-ad8be0453d34",
      "path": "/springcloud/",
      "lastUpdated": 1550048302000,
      "title": "SpringCloud入门",
      "headers": [
        {
          "level": 2,
          "title": "SpringCloud介绍",
          "slug": "springcloud介绍"
        },
        {
          "level": 2,
          "title": "SpringCloud组件",
          "slug": "springcloud组件"
        },
        {
          "level": 2,
          "title": "SpringCloud发展",
          "slug": "springcloud发展"
        },
        {
          "level": 3,
          "title": "SpringCloud第一代",
          "slug": "springcloud第一代"
        },
        {
          "level": 3,
          "title": "SpringCloud 第二代",
          "slug": "springcloud-第二代"
        },
        {
          "level": 2,
          "title": "SpringCloud开源推荐",
          "slug": "springcloud开源推荐"
        }
      ],
      "frontmatter": {
        "title": "SpringCloud入门",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "SpringCloud介绍"
          },
          {
            "name": "keywords",
            "content": "SpringCloud介绍"
          }
        ]
      }
    },
    {
      "key": "v-debe3f600a9e4",
      "path": "/springcloud/springcloud-nacos.html",
      "lastUpdated": 1549964418000,
      "title": "Nacos介绍",
      "headers": [
        {
          "level": 2,
          "title": "Nacos介绍",
          "slug": "nacos介绍"
        },
        {
          "level": 2,
          "title": "安装阿里nacos",
          "slug": "安装阿里nacos"
        },
        {
          "level": 2,
          "title": "搭建nacos服务端",
          "slug": "搭建nacos服务端"
        },
        {
          "level": 2,
          "title": "启动nacos服务端",
          "slug": "启动nacos服务端"
        }
      ],
      "frontmatter": {
        "text": "微服务注册中心-阿里nacos",
        "lang": "zh-CN",
        "meta": [
          {
            "name": "description",
            "content": "微服务注册中心-阿里nacos"
          },
          {
            "name": "keywords",
            "content": "微服务注册中心-阿里nacos"
          }
        ]
      }
    },
    {
      "key": "v-66d6d90c2809d",
      "path": "/springcloud/springcloud2.html",
      "lastUpdated": 1549859269000,
      "title": "微服务-配置中心",
      "headers": [
        {
          "level": 2,
          "title": "开始学习springcloud 配置中心",
          "slug": "开始学习springcloud-配置中心"
        }
      ],
      "frontmatter": {
        "title": "微服务-配置中心 "
      }
    },
    {
      "key": "v-08e57cf5bd432",
      "path": "/vue/",
      "lastUpdated": 1549859269000,
      "title": "vue带你走进前端新纪元",
      "frontmatter": {
        "title": "vue带你走进前端新纪元"
      }
    },
    {
      "key": "v-013ef341b53f1",
      "path": "/vue/vue1.html",
      "lastUpdated": 1549964418000,
      "title": "vue 第一课",
      "headers": [
        {
          "level": 2,
          "title": "vue 介绍",
          "slug": "vue-介绍"
        }
      ],
      "frontmatter": {
        "title": "vue 第一课"
      }
    },
    {
      "key": "v-7f015efd66578",
      "path": "/vue/vue2.html",
      "lastUpdated": 1549859269000,
      "title": "vue 第二课",
      "headers": [
        {
          "level": 2,
          "title": "vue 相关组件",
          "slug": "vue-相关组件"
        }
      ],
      "frontmatter": {
        "title": "vue 第二课"
      }
    }
  ],
  "themeConfig": {
    "repo": "https://github.com/mrjerryli/myblog.git",
    "repoLabel": "GitHub",
    "docsRepo": "https://github.com/mrjerryli/myblog",
    "docsDir": "docs",
    "docsBranch": "gh-pages",
    "editLinks": false,
    "displayAllHeaders": false,
    "activateHeaderLinks": false,
    "sidebarDepth": 2,
    "locales": {
      "/": {
        "label": "简体中文",
        "selectText": "选择语言",
        "editLinkText": "编辑此页",
        "lastUpdated": "上次更新",
        "serviceWorker": {
          "updatePopup": {
            "message": "发现新内容可用",
            "buttonText": "刷新"
          }
        }
      }
    },
    "nav": [
      {
        "text": "🚀首页",
        "link": "/"
      },
      {
        "text": "📗旧博客",
        "link": "https://www.lijiating.online",
        "icon": "reco-tag"
      },
      {
        "text": "目录",
        "items": [
          {
            "text": "开源学习",
            "items": [
              {
                "text": "资源链接",
                "link": "/resources/resources"
              },
              {
                "text": "开源书籍",
                "link": "/resources/books"
              }
            ]
          },
          {
            "text": "前端学习",
            "items": [
              {
                "text": "javascript",
                "link": "/js/"
              },
              {
                "text": "css",
                "link": "/css/"
              },
              {
                "text": "vue",
                "link": "/vue/"
              }
            ]
          },
          {
            "text": "后端学习",
            "items": [
              {
                "text": "java",
                "link": "/java/"
              },
              {
                "text": "springboot",
                "link": "/springboot/"
              },
              {
                "text": "springcloud",
                "link": "/springcloud/"
              }
            ]
          },
          {
            "text": "学习扩展",
            "items": [
              {
                "text": "三方教程",
                "link": "/otherProse/"
              }
            ]
          }
        ]
      },
      {
        "text": "👓在线文档",
        "items": [
          {
            "text": "WEB前端",
            "items": [
              {
                "text": "Vue",
                "link": "https://cn.vuejs.org/v2/api/"
              },
              {
                "text": "Vuex",
                "link": "https://vuex.vuejs.org/zh/api/#vuex-store"
              },
              {
                "text": "Axios",
                "link": "https://www.kancloud.cn/yunye/axios/234845"
              },
              {
                "text": "Layout",
                "link": "http://www.jeasyui.net/plugins/162.html"
              },
              {
                "text": "Jquery",
                "link": "https://www.jquery123.com/"
              },
              {
                "text": "腾讯云开发者手册",
                "link": "https://cloud.tencent.com/developer/devdocs"
              }
            ]
          },
          {
            "text": "SERVER后端",
            "items": [
              {
                "text": "SpringBoot开发文档",
                "link": "https://docs.spring.io/spring-boot/docs/2.1.2.RELEASE/reference/htmlsingle/"
              },
              {
                "text": "SpringCloud-F",
                "link": "https://cloud.spring.io/spring-cloud-static/Finchley.SR2/single/spring-cloud.html"
              },
              {
                "text": "springboot2.0.5中文开发文档",
                "link": "http://springboot.lianmengtu.top"
              },
              {
                "text": "收集各种Spring Cloud 学习资源",
                "link": "/springcloud/AwesomeSpringCloud"
              }
            ]
          },
          {
            "text": "移动端",
            "items": [
              {
                "text": "Flutter",
                "link": "https://flutterchina.club/docs"
              }
            ]
          },
          {
            "text": "评论系统",
            "items": [
              {
                "text": "valine无后端评论系统",
                "link": "https://valine.js.org"
              }
            ]
          }
        ]
      },
      {
        "text": "👓在线工具",
        "items": [
          {
            "text": "数据",
            "items": [
              {
                "text": "JSON在线编辑",
                "link": "https://www.bejson.com/jsoneditoronline"
              },
              {
                "text": "MD表格在线编辑",
                "link": "https://tool.lu/tables/"
              },
              {
                "text": "Cron表达式在线生成",
                "link": "http://cron.qqe2.com"
              },
              {
                "text": "代码格式化",
                "link": "http://tool.oschina.net/codeformat/html"
              },
              {
                "text": "UUID在线生成",
                "link": "https://1024tools.com/uuid"
              },
              {
                "text": "在线properties转yaml、yml工具",
                "link": "https://www.bejson.com/devtools/properties2yaml/"
              },
              {
                "text": "在线工具集",
                "link": "http://tool.oschina.net"
              },
              {
                "text": "BASE64加密解密",
                "link": "http://tool.oschina.net/encrypt?type=3"
              },
              {
                "text": "gitignore忽略文件在线生成",
                "link": "https://www.gitignore.io"
              }
            ]
          },
          {
            "text": "开源仓库",
            "items": [
              {
                "text": "maven仓库",
                "link": "https://mvnrepository.com"
              },
              {
                "text": "gradle",
                "link": "https://docs.gradle.org/5.2/javadoc/index.html"
              }
            ]
          }
        ]
      },
      {
        "text": "职业生涯计划",
        "items": [
          {
            "text": "2019年学习计划",
            "link": "/plan/2019plan"
          }
        ]
      },
      {
        "text": "🌈分享链接",
        "items": [
          {
            "text": "GitHub",
            "link": "https://github.com/mrjerryli"
          },
          {
            "text": "码云",
            "link": "https://gitee.com/ljtJerry"
          },
          {
            "text": "D2admin",
            "link": "https://doc.d2admin.fairyever.com/zh/"
          },
          {
            "text": "技术胖",
            "link": "https://jspang.com"
          },
          {
            "text": "爱搜资源",
            "link": "https://www.aisouziyuan.com"
          }
        ]
      },
      {
        "text": "个人介绍",
        "link": "/about/"
      }
    ],
    "serviceWorker": {
      "updatePopup": {
        "message": "发现新内容可用",
        "buttonText": "刷新"
      }
    },
    "sidebar": {
      "/css/": [
        {
          "title": "样式学习",
          "collapsable": true,
          "children": [
            "css1",
            "css2"
          ]
        }
      ],
      "/otherProse/": [
        {
          "title": "Gogs服务器安装教程",
          "collapsable": true,
          "children": [
            "gogsInstall"
          ]
        },
        {
          "title": "markdown扩展",
          "collapsable": true,
          "children": [
            "markdown扩展"
          ]
        }
      ],
      "/resources/": [
        {
          "title": "开源资源",
          "collapsable": true,
          "children": [
            "resources",
            "books"
          ]
        }
      ],
      "/plan/": [
        {
          "title": "职业生涯计划",
          "children": [
            "/plan/2019plan"
          ]
        }
      ],
      "/js/": [
        {
          "title": "javascript学习",
          "children": [
            "/js/js1",
            "/js/js2"
          ]
        }
      ],
      "/vue/": [
        {
          "title": "vue学习",
          "children": [
            "/vue/vue1",
            "/vue/vue2"
          ]
        }
      ],
      "/java/": [
        {
          "title": "开始学习java",
          "children": [
            "/java/java1",
            "/java/java2"
          ]
        }
      ],
      "/springboot/": [
        {
          "title": "开始学习springboot",
          "children": [
            "/springboot/springboot1",
            "/springboot/springboot2"
          ]
        }
      ],
      "/springcloud/": [
        {
          "title": "微服务前言",
          "children": [
            "/springcloud/"
          ]
        },
        {
          "title": "微服务架构之nacos",
          "children": [
            "/springcloud/springcloud-nacos"
          ]
        },
        {
          "title": "微服务架构···",
          "children": [
            "/springcloud/springcloud2"
          ]
        }
      ]
    }
  },
  "locales": {
    "/": {
      "lang": "zh-CN",
      "title": "李加廷の博客",
      "description": "努力攒够了，梦想才会开花！🌼"
    }
  }
}